      ...
      <div className="p-6 space-y-8 flex-1">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { id: ModelType.FLASH, label: 'Nano Flash', sub: 'Rápido & Simples', icon: <Icons.Zap size={14} /> },
              { id: ModelType.PRO, label: 'Nano Pro', sub: 'Exige Chave Paga', icon: <Icons.Star size={14} /> }
            ].map((m) => (
              <button
                key={m.id}
          ...
          {config.model === ModelType.PRO && (
            <p className="text-[9px] text-amber-600 font-bold bg-amber-50 p-2 rounded-lg border border-amber-100">
              * O modo Nano Pro requer uma chave API de um projeto Google Cloud faturado.
            </p>
          )}
        </div>
